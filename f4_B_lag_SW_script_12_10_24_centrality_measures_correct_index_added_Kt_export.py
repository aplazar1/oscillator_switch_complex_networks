...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov  9 16:53:30 2024
...
#Model default params in the order described below: 200,30,0.03,1,4,100,10,3.2,0,100,5,1,1.5,1,0.01,50
...
print("enter  simulation parameters: total_nodes, k, p, rep, K_t_init, K_theta_max,Kx_theta, Kxx, mean_omega, mean_x_init, sigma_omega, SD_x_init, eta, tau, dt, secs, respectively, separated by commas")
total_nodes,k, p, rep, K_t_init, K_theta_max,Kx_theta, Kxx, mean_omega, mean_x_init, sigma_omega, SD_x_init, eta, tau, dt, secs =input().split(',')
...
G1=nx.watts_strogatz_graph(n=total_nodes, k=k, p=p) ##use a single graph for all the types of nodes
...
SW_p_K_r_theta_subgraph_log.write("kmax1")
SW_p_K_r_theta_subgraph_log.write(" ")
...
fourier_correlation_analysys_SW=open("SW_fourier_analysis_10_30_24.tsv", "a")
fourier_correlation_analysys_SW.write('frequency domain xf')
fourier_correlation_analysys_SW.write("\t")
fourier_correlation_analysys_SW.write("r_theta_fft")
fourier_correlation_analysys_SW.write("\t")
fourier_correlation_analysys_SW.write("psi_fft")
if max(r_x_list)> 0: 
    fourier_correlation_analysys_SW.write("\t")
    fourier_correlation_analysys_SW.write("rx_fft")
    fourier_correlation_analysys_SW.write("\t")
    fourier_correlation_analysys_SW.write("<x(t)> fft")
fourier_correlation_analysys_SW.write("\n")


for t in range(len(xf)):
    fourier_correlation_analysys_SW.write(str(xf[t]))
    fourier_correlation_analysys_SW.write("\t")
    fourier_correlation_analysys_SW.write(str(r_theta_fft[t]))
    fourier_correlation_analysys_SW.write("\t")
    fourier_correlation_analysys_SW.write(str(psi_fft[t]))
 
    if max(r_x_list)> 0: 
        fourier_correlation_analysys_SW.write("\t")
        fourier_correlation_analysys_SW.write(str(rx_fft[t]))
        fourier_correlation_analysys_SW.write("\t")
        fourier_correlation_analysys_SW.write(str(x_fft[t]))
    fourier_correlation_analysys_SW.write("\n")
...
SW_p_K_r_theta.write("\t")
SW_p_K_r_theta.write("mean_omega")
SW_p_K_r_theta.write("\t")
SW_p_K_r_theta.write("sigma_omega")
SW_p_K_r_theta.write("\t")
...
SW_p_K_r_theta.write("\t") 
SW_p_K_r_theta.write(str(kmax_node_type))
SW_p_K_r_theta.write("\t")  
...
SW_hub_order_params=open("SW_hub_order_params.tsv", "a")
SW_hub_order_params.write("switch-switch subgraph hub complex <rx_eq>")
SW_hub_order_params.write("\t")
SW_hub_order_params.write("oscillator-oscillator hub subgraph complex <rtheta_eq>")
SW_hub_order_params.write("\t")
SW_hub_order_params.write("full graph hub hub complex <rx_eq>")
SW_hub_order_params.write("\t")
SW_hub_order_params.write("full graph hub rtheta complex <rx_eq>")
SW_hub_order_params.write("\n")

if len(np.shape(rx_hub_switch_subgraph))>1:
    SW_hub_order_params.write(str(np.mean(np.mean(rx_hub_switch_subgraph[:-1000], axis=1), axis=0)))
    SW_hub_order_params.write("\t")
else:
    SW_hub_order_params.write(str(np.mean(rx_hub_switch_subgraph[:-1000])))
    SW_hub_order_params.write("\t")

if len(np.shape(r_theta_hub_oscillator_subgraph))>1:
    SW_hub_order_params.write(str(np.mean(np.mean(r_theta_hub_oscillator_subgraph[:-1000], axis=1), axis=0)))
    SW_hub_order_params.write("\t")
else:
    SW_hub_order_params.write(str(np.mean(r_theta_hub_oscillator_subgraph[:-1000])))
    SW_hub_order_params.write("\t")
    
if len(np.shape(rx_hub_overall))>1:
    SW_hub_order_params.write(str(np.mean(np.mean(rx_hub_overall[:-1000], axis=1), axis=0)))
    SW_hub_order_params.write("\t")
else:
    SW_hub_order_params.write(str(np.mean(rx_hub_overall[:-1000])))
    SW_hub_order_params.write("\t")

if len(np.shape(r_theta_hub_overall))>1:
    SW_hub_order_params.write(str(np.mean(np.mean(r_theta_hub_overall[:-1000], axis=1), axis=0)))
    SW_hub_order_params.write("\n")
else:
    SW_hub_order_params.write(str(np.mean(r_theta_hub_overall[:-1000])))
    SW_hub_order_params.write("\n")


SW_p_K_r_theta.write(str(K_t_init))
SW_p_K_r_theta.write("\t")
SW_p_K_r_theta.write(str(mean_omega))
SW_p_K_r_theta.write("\t")
SW_p_K_r_theta.write(str(sigma_omega))
SW_p_K_r_theta.write("\t")
...
order_param_corr_SW=open("SW_order_param_and_correlation_timeseries_analysis_10_30_24.tsv", "a")
order_param_corr_SW.write('model time units')
order_param_corr_SW.write('\t')
order_param_corr_SW.write('K(t)')
order_param_corr_SW.write('\t')
order_param_corr_SW.write("r_theta")
order_param_corr_SW.write("\t")
order_param_corr_SW.write("r_x")
order_param_corr_SW.write("\t")
order_param_corr_SW.write("r_theta_autocorr")
if max(r_x_list)> 0: 
    order_param_corr_SW.write("\t")
    order_param_corr_SW.write("rx_autocorr")
    order_param_corr_SW.write("\t")
    order_param_corr_SW.write("order_param_cross_corr")
order_param_corr_SW.write("\n")


for t in range(T):
    order_param_corr_SW.write(str(time[t]))
    order_param_corr_SW.write("\t")
    order_param_corr_SW.write(str(K_t[t]))
    order_param_corr_SW.write('\t')
    order_param_corr_SW.write(str(r_theta_list[t]))
    order_param_corr_SW.write("\t")
    order_param_corr_SW.write(str(r_x_list[t]))
    order_param_corr_SW.write("\t")
    order_param_corr_SW.write(str(r_theta_auto_corr[t]))
 
    if max(r_x_list)> 0: 
        order_param_corr_SW.write("\t")
        order_param_corr_SW.write(str(rx_auto_corr[t]))
        order_param_corr_SW.write("\t")
        order_param_corr_SW.write(str(order_param_crosscorr[t]))
    order_param_corr_SW.write("\n")

SW_p_K_r_theta.close()
SW_p_K_r_theta_subgraph_log.close()
fourier_correlation_analysys_SW.close()
SW_hub_order_params.close()
order_param_corr_SW.close()
